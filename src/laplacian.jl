...
function laplacian_mpo(s::Vector{<:Index}, xstep = 1.0)
    L = length(s)
    l = [Index(3, "l=$(j)↔$(j+1)") for j = 0:L]
    T⃗ = [itensor(laplacian_tensor(), l[j], l[j+1], s[j], s[j]') for j = 1:L]
...
function laplacian_mpo(
    s::Tuple{Vector{<:Index},Vector{<:Index}},
    xstep::Tuple{Number,Number} = (1.0, 1.0),
)
    Δ₁ = interleave(laplacian_mpo(s[1], xstep[1]), MPO(s[2], "I"))
    Δ₂ = interleave(MPO(s[1], "I"), laplacian_mpo(s[2], xstep[2]))
    return convert(
        MPO,
        +(insert_missing_links(Δ₁), insert_missing_links(Δ₂); alg = "directsum"),
    )
end
...
function s_generic_mpo(f, s)
    n = length(s)
    l = [Index(2, "l=$n↔$(n + 1)") for n = 0:(n+1)]
    M = MPO([f(l[j], l[j+1], s[j], s[j]') for j = 1:n])
    M[1] *= onehot(l[1] => 1)
    M[n] *= onehot(l[n+1] => 2)
    return M
end
